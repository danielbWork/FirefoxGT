 ...
 */
 setupOnClickHandler() {
  ...
  browser.contextMenus.onClicked.addListener(this.onToggleContextMenu.bind(this));
}
 ...
 */
private async onGroupTabActivated(activeInfo: Tabs.OnActivatedActiveInfoType) {
  let groupTab = await StorageHandler.instance.getGroupTabByID(activeInfo.tabId);
 ...
 * @param previousTabId The id of tab this was moved from
 */
private async findNewActiveTab(groupTab: GroupTab, previousTabId: number | undefined) {
  ...
  const validTabs = otherTabs.filter((value) => {
    ...
    return id && !groupTabIDs.includes(`${id}`) && !groupTab.innerTabs.includes(id);
  });
