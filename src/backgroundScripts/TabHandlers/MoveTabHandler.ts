...
setupMoveHandler() {
  ...
  browser.contextMenus.onClicked.addListener(this.onMoveInnerTabMenuItemClick.bind(this));
}
 ...
 */
 private async onTabMoved(tabId: number, moveInfo: Tabs.OnMovedMoveInfoType) {
  let { groupTab, index } = await StorageHandler.instance.getGroupTabOrInnerTabByID(tabId);
 ...
 */
private async onMoveInnerTabMenuItemClick(info: Menus.OnClickData, tab?: Tabs.Tab) {
  ...
  if (info.menuItemId.startsWith(MOVE_TO_GROUP_TAB_ID)) {
    ...
    this.moveTabFromGroupToNewGroupMenuClick(groupTab, index, parseInt(newGroupID));
  }
 ...
 */
private async moveGroupTab(groupTab: GroupTab, index: number, additionalTabs : number[] = []) {
 ...
 */
private async onGroupTabMove(groupTab : GroupTab, moveInfo: Tabs.OnMovedMoveInfoType) {
 ...
 * @param moveInfo Info regarding the tab movement
 */
private async onMoveIntoGroupTab(tabId: number, moveInfo: Tabs.OnMovedMoveInfoType) {
  const { groupTab, groupTabInfo } = await checkMovedIntoGroupTab(moveInfo.toIndex);
  ...
  if (groupTab && groupTabInfo) {
    ...
    if (results[0]) {
      StorageHandler.instance.addInnerTab(groupTab, tabId, moveInfo.toIndex - groupTabInfo.index - 1);
 ...
 */
private async onInnerTabMove(groupTab: GroupTab, index: number, moveInfo:Tabs.OnMovedMoveInfoType) {
  ...
  if (
    ...
    await this.onReorderInGroupTab(groupTab, groupTabInfo.index, index, moveInfo);
    return;
  }
...
private async onMoveFromOneGroupToOther(
  ...
  const movedTabInfo = await tabs.get(
 ...
 * @param innerTabIndex The index of the inner tab in the group tab
 */
private async onRemoveTabFromGroup(groupTab: GroupTab, groupTabInfo: Tabs.Tab, innerTabIndex: number) {
  const movedTabInfo = await tabs.get(
    groupTab.innerTabs[innerTabIndex]
  );
...
private async removeTabFromGroupMenuClick(groupTab: GroupTab, id: number) {
  ...
  const updatedGroupTab = (await StorageHandler.instance.getGroupTabByID(groupTab.id))!;
